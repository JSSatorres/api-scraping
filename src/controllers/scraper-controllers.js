  ...
  try {
    (async () => {
      ...
      for (const item of listAuthorComents) {
        ...
        dataAuthorComents.push({ author: getAuthor,comments: "null" , control:"1" });
      }

      const info = dataTitleUrl.map((titleComent, index) => {
        return {title: titleComent?.title,url: titleComent?.url, author: dataAuthorComents[index]?.author, comments: dataAuthorComents[index]?.comments};
      });
